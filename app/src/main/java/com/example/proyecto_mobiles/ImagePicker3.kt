...
class ImagePicker3(private val activity: Activity) {
    ...
    fun onActivityResult(requestCode3: Int, resultCode3: Int, data3: Intent?): Bitmap? {
        if (requestCode3 == MY_REQUEST_PICK_IMAGE) {
            if (resultCode3 != Activity.RESULT_OK) {
                return null
            }
            val uri = data3?.data
            if (uri != null) {
    ...
    companion object {
        const val MY_REQUEST_PICK_IMAGE = 1002
